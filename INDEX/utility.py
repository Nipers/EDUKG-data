...
IMPLICIT_CLASSES  = ['http://www.w3.org/2002/07/owl#NamedIndividual', 'http://www.w3.org/2002/07/owl#DatatypeProperty', 'http://www.w3.org/2002/07/owl#Class', 'http://www.w3.org/2002/07/owl#ObjectProperty', 'http://www.openannotation.org/ns/Annotation', 'http://edukb.org/knowledge/0.1/class#Category', 'http://xmlns.com/foaf/0.1/Person']
IMPLICIT_CLASSES_NAME = ['w3-NamedIndividual', 'w3-DataProperty', 'w3-Class', 'w3-ObjectProperty', 'openannotation-annotation', 'edukg-Category', 'xmlns-Person']
...
NAMESPACE2URI = {
    ...
    'owl': OWL,
    **{
        'edukg_%s_%s' % (u, course): EDUKG_PROP[course+'#'] if u == 'prop' else EDUKG_CLASS[course+'#'] if u == 'cls' else EDUKG_INS[course+'#']
...
isEntity = lambda v: isinstance(v, str) and (v in IMPLICIT_CLASSES or v.startswith('http://edukb.org/') or v.startswith('http://www.w3.org') or v.startswith('http://edukg.org/'))
...
def read_csv_triplets(file_path):
    ...
    for err_segment in err_segments:
        new_items.append([items[err_segment[0]][0], items[err_segment[0]][1], '\n'.join([items[err_segment[0]][2]] + ['\t'.join(items[i]) for i in range(err_segment[0] + 1, err_segment[1])])])
...
#         suffixes = [getSuffix(v).split('/') for v in entity_domains if isinstance(v, str) and entity_domains[v] == domain]
...
#     predicate_domains_cnt = {v:list(predicate_domains.values()).count(v) for v in tqdm(set(predicate_domains.values()))}
...
#     object_properties, data_properties = [v for v in tqdm(predicate_samples) if predicate_samples[v]['ratio'] > .20], [v for v in tqdm(predicate_samples) if predicate_samples[v]['ratio'] <= .20]
...
#         suffixes = [getSuffix(v).split('/') for v in predicate_domains if isinstance(v, str) and predicate_domains[v] == domain]
...
def analyze_subdomain(suffixes):
    ...
    if all([v == 1 for v in suffix_num[:-1]]) or len(suffix_num) <= 1:
        ...
        start_i = min([v for v in range(len(suffix_num)) if suffix_num[v] != 1])  # Find the first index in suffix_num that its element does not equal to 1
...
def build_properties_dataframe(nodes, relations, info):
    ...
    data_properties, object_properties = [v for v in info['relation']['relation_samples'] if info['relation']['relation_samples'][v]['type'] == 'data_property'], [v for v in info['relation']['relation_samples'] if info['relation']['relation_samples'][v]['type'] == 'object_property']
    data_properties, object_properties = pd.DataFrame({'uri': data_properties}), pd.DataFrame({'uri': object_properties})
    ...
    data_properties['entity_num'] = data_properties['uri'].apply(lambda x: int(info['relation']['relation_samples'][x]['num'] * info['relation']['relation_samples'][x]['ratio']))
    object_properties['entity_num'] = object_properties['uri'].apply(lambda x: int(info['relation']['relation_samples'][x]['num'] * info['relation']['relation_samples'][x]['ratio']))
    ...
    for i in tqdm(range(object_properties.shape[0])):
        ...
        for j in tqdm(range(chosen.shape[0])):
            ...
            name_s, name_o = nodes.loc[value_s, 'name'], nodes.loc[value_o, 'name'] if isEntity(value_o) and 'category' not in value_o else value_o
            if 'annotation' in predicate or (isinstance(name_s, str) and isinstance(name_o, str) and len(name_s) > 0 and len(name_o) > 0 and name_s != 'Unknown' and name_o != 'Unknown'):
    ...
    for i in tqdm(range(data_properties.shape[0])):
        ...
        if isinstance(predicate, str) and ('edukg' in predicate_domain or 'edukb' in predicate_domain):
            ...
            for j in range(chosen.shape[0]):
                ...
                if isinstance(name_s, str) and isinstance(name_o, str) and len(name_s) > 0 and len(name_o) > 0 and name_s != 'Unknown' and name_o != 'Unknown':
...
def get_k_fold_dataset_by_class(data, dtype = 'seed_df', label_key='class', n=2):
    ...
    result = []
    if dtype == 'seed_df':
        data['instance_list'] = data['instances'].apply(lambda x: [v for v in x.split(',')] if isinstance(x, str) else [])
        ...
        x, y = np.array([ins for cls in data for ins in data[cls]]), np.array([cls for cls in data for ins in data[cls]])
...
def get_LCA_pairs(g, m_list, n_list, search_limit=None):
    product_results = pd.DataFrame([res for v in product(m_list, n_list) for res in lowest_common_ancestor(g, *v, search_limit=search_limit)], columns=['uri', 'm_length', 'n_length'])
    if product_results.shape[0] == 0:
        return []
    product_results['score'] = product_results.apply(lambda x: 169/ (x['m_length'] * x['n_length']) * np.log(g.nodes[x['uri']]['depth']) / len(list(g.predecessors(x['uri']))), axis=1)
    return sorted(product_results.groupby('uri')['score'].apply(sum).to_dict().items(), key=lambda x:x[1], reverse=True)
...
def get_type_LCAs(g, classdict):
    ...
    return sorted(pd.DataFrame([i for v in results for i in v], columns=['uri', 'score']).groupby('uri')['score'].apply(sum).to_dict().items(), key=lambda x:x[1], reverse=True)
...
def lowest_common_ancestors(g, class_cnt, root_class='http://xlore.org/concept/zhc290957'):
    ...
    score_by_path = {
        v: [
            (uri, 169 / ((g.nodes[v]['depth'] - g.nodes[uri]['depth']) ** 2 + 1) * np.log(g.nodes[uri]['depth']) / (len(list(g.predecessors(uri))) + 1) * class_cnt[v])
            for uri in paths[v]]
        for v in paths
    }
    return sorted(pd.DataFrame([v for k in score_by_path for v in score_by_path[k]], columns=['uri', 'score']).groupby('uri')['score'].apply(lambda x: np.sqrt(x.prod())).to_dict().items(), key=lambda x:x[1], reverse=True)