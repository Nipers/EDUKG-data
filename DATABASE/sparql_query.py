...
def get_edukg_triplets(graph_iri):
    ...
    for offset in tqdm(range(0, limit, 10000)):
        ...
        sparql.setReturnFormat(JSON)
        results += [(triple['s']['value'], triple['p']['value'], triple['o']['value']) for triple in sparql.query().convert()['results']['bindings']]
...
from utility import remove_puncts, TYPE_PREDICATE, LABEL_PREDICATE, CLASS_PREDICATE, COURSES_ZH, COURSES_EN, TO_ZH_TR, TO_ZH_SI, NAMESPACE2URI
...
GET_CLASSES = lambda x: [{'uri': v['object']['value'], 'name': v['oname']['value'] if 'oname' in v else None} for v in x['results']['bindings'] if v['predicate']['value'] == CLASS_PREDICATE ] if x is not None else []
...
class JenaConnector():
    ...
    def find_subtree_instances(self, uri, search_layer=5, max_instances=1000):
        ...
        for c in result:
            ...
            result[c]['instances'] = random.sample(bindings, max_instances) if len(bindings) > max_instances else bindings
        return g, result
    ...
    def get_json(self, uri):
        ...
        result['polysemys'] = triplet_dict['http://xlore.org/property/supplement'][0][0] if 'http://xlore.org/property/supplement' in triplet_dict else ''
        ...
        result['abstracts'] = triplet_dict['http://www.w3.org/2000/01/rdf-schema#comment'][0][0] if 'http://www.w3.org/2000/01/rdf-schema#comment' in triplet_dict else ""
        infobox = {
            k:triplet_dict[k] for k in triplet_dict if 'comment' not in k and 'isRelatedTo' not in k and 'property' in k and 'hasURL' not in k and 'supplement' not in k
        }
        for k in infobox:
        ...
        if fuzzy:
            ...
            if query_result is not None:
                ...
                if not keep_accurate:
                    return [v['subject']['value'] for v in sorted_result], all([v['label']['value'] == name for v in sorted_result])
                else:
                    if sorted_result[0]['label']['value'] == name:
                        return [v['subject']['value'] for v in sorted_result if v['label']['value'] == name], True
                    else:
                        return [v['subject']['value'] for v in sorted_result[:int(len(sorted_result) * keep_partial)]], all([v['label']['value'] == name for v in sorted_result])
        else:
            if mode not in MODE2FUNC:
                raise BaseException("The query word 'mode' should be in ['both', 'zh-tr', 'zh-si']")
            if base not in ['zh', 'bd', 'en', 'all']:
    ...
    def get_property_triplet(self, uri):
        ...
        if len(query_result['results']['bindings']):
            return {k:query_result['results']['bindings'][0][k]['value'] for k in query_result['results']['bindings'][0]}
        else:
            return {}
        ...
        for label in labels:
            for subject in COURSES_ZH:
                ...
                while True:
                    ...
                    if result['code'] == '0':
                        contents = [BeautifulSoup('\n'.join(v['content'])).text if v['content'] else '' for v in result['data']]
                        if not fuzzy:
...
if __name__ == '__main__':
    ...
    for course in COURSES_EN:
        ...
        result = {course_classes.iloc[i]['uri']:{'name':course_classes.iloc[i]['name'], 'instances':[r['n']['name'] for r in neo4j_connector.query_by_label(course_classes.iloc[i]['uri']) if isinstance(r['n']['name'], str) and r['n']['name'] != 'Unknown']} for i in tqdm(range(course_classes.shape[0]))}
        ...
        result_df = pd.DataFrame(
            [[result[uri]['name'], ','.join(result[uri]['query_result'].keys()), len(result[uri]['query_result']), uri] for uri in result],
    ...
    matched_cls['instances'] = matched_cls['zh-cls-uri'].progress_apply(lambda x: connector.get_class_descendant_children(x, layer=5))
    ...
    instance_list = [(cls_obj['class'], k, cls_obj['instances'][k]) for cls_obj in matched_cls_list for k in cls_obj['instances']]
    ...
    sampled_instances = instances_df[instances_df['abstract'].apply(lambda x: len(x) > 10)].groupby('y')['abstract'].apply(lambda x:random.sample(x.tolist(), min(10, len(x)))).to_dict()
    pd.DataFrame([(y, v) for y in sampled_instances for v in sampled_instances[y]], columns=['y', 'name']).to_excel('tmp_file/history_class_matching_sampled_augmented_data.xlsx', index=False)
